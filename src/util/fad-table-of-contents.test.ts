...
describe('getFadTableOfContents', () => {
	it('for narrative pages', () => {
		const dom = sync(
			fs.readFileSync(
				path.resolve(
					__dirname,
					'../../test/xml/fad/narrative-headings-test.xml'
				),
				'utf8'
			)
		);
		...
		expect(
			getFadTableOfContents((dom as unknown) as Document).map(
				stripNodeFromToc
			)
		).toMatchSnapshot();
	});
	it('for React component type pages', () => {
		const dom = sync(
			fs.readFileSync(
				path.resolve(__dirname, '../../test/xml/fad/Button.xml'),
				'utf8'
			)
		);
		// console.log(slimdom.serializeToWellFormedString(dom));
	...
	it('for an operation', () => {
		const dom = sync(
			fs.readFileSync(
				path.resolve(
					__dirname,
					'../../test/xml/fad/horizontal-insert.xml'
				),
				'utf8'
			)
		);
	...
	it('for a class with both methods and properties', () => {
		const dom = sync(
			fs.readFileSync(
				path.resolve(
					__dirname,
					'../../test/xml/fad/BlueprintRange.xml'
				),
				'utf8'
			)
		);
